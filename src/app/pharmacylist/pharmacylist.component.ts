...
export class PharmacylistComponent implements OnInit {
  ...
  constructor(private _http: PharmacylistService, private modalService: NgbModal, private toastr: ToastrService) { }
  ...
  open(basic) {
    ...
    this.total = 0;
    this.returnSales = this.newReturn.refSalesReturns.filter(sale => Object.keys(sale).length !== 0);
    ...
    if (this.returnSales.length == 0) { //new line added by anji
  ...
  showPdf() {
    ...
    this._http.getPharmacypdf(this.regNo)
      .subscribe(
      data => {
  ...
  showAllpharmacylist() {
    ...
    this._http.getPharmacydetails(this.regNo)
      .subscribe(
      data => {
        ...
        data[1].map(res => {
          let saleDetail: SaleDetail = new SaleDetail(res.BillNo, res.MedId, res.MedName, +(res.mrp), res.Quantity, 0, res.discount, res.Paid, 0, res.NetAmount, res.batch, res.previousPaymentType);
  ...
  save() {
    ...
    this._http.editbill(this.regNo, this.newReturn, )
