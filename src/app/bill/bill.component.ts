...
import { Bill, edit } from "src/app/bill/bill.model";
...
Edit:edit[]=[]
  ...
  constructor(private fb:FormBuilder,private _http:BillService,private modalService: NgbModal,private toastr: ToastrService) { 
...
this.cardamount=(this.ccamount-(this.cashamount ? this.cashamount : 0))
  ...
  showAppBill(regId) {
      ...
      this._http.getAppBill(this.regId) 
        .subscribe(
        data => {
  ...
  showDischarge(regId) {
    let param={
      ...
      this._http.dischargeSlip(this.regId,param) 
        .subscribe(
        data => {
    ...
    showDetailBill(regId) {
        ...
        this._http.getDetaillBill(this.regId) 
          .subscribe(
          data => {
...
chargeName:string
  ...
  this._http.getMed(data.chargeName,this.regId)
  .subscribe(
  response => {
...
//  if(data.discount || data.discount<=0)
 data['netAmount']= data['amount']-(data['amount']*(data.discount)/100.0);
    ...
    edit2(editpopup) {
      ...
      this.modalService.open(editpopup, { ariaLabelledBy: 'modal-basic-title',size:'lg' }).result.then((result) => {
        this.closeResult = `Closed with: ${result}`;
      }, (reason) => {
        this.closeResult = `Dismissed ${this.getDismissReason(reason)}`;
      });
...
this.add =Math.round(((this.total?this.total:0)+(this.total2?this.total2:0))-((this.advanceamount?this.advanceamount:0)))
    ...
    // this.add = this.total+this.total2
            ...
            this.modalService.open(basic2, { ariaLabelledBy: 'modal-basic-title',size:'sm'}).result.then((result) => {
              this.closeResult = `Closed with: ${result}`;
            }, (reason) => {
              this.closeResult = `Dismissed ${this.getDismissReason(reason)}`;
            });
    ...
    caldiscount()
    {
      if(((this.total?this.total:0)+(this.total2?this.total2:0))>=((this.advanceamount?this.advanceamount:0))){
      ...
      this.returnf=((this.advanceamount?this.advanceamount:0))-((this.total?this.total:0)+(this.total2?this.total2:0))
    }
    addMore(event) { 
   ...
   let param={
        ...
        "updateCharge":this.Edit
                ...
                this._http.editbill(this.regId,param)
                  .subscribe(
                  data => {
    ...
    advancepdf(){
      this._http.getAdvance(this.regId)
      .subscribe(
        data => {
      ...
      block(){
        let flag ="true"
        this._http.blockPatient(this.regId,flag)
        .subscribe(
          data => {
