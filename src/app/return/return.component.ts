...
export class ReturnComponent implements OnInit {
  ...
  getBillDetails(event) {
    ...
    this._http.getDetails(this.billDetail.billNo)
      ...
      response => {
        ...
        this.newReturn.location = this.billDetail.location = response[0].location;
        ...
        response.map(res => {
          let saleDetail: SaleDetail = new SaleDetail(res.medicineName, res.mrp, res.batchNo, res.discount, res.quantity, 0, 0, res.gst, res.expireDate);
  ...
  save() {
    ...
    this._http.editBill(this.newReturn)
