...
detail: Details[] = [];
minDate = new Date();
  constructor(private fb:FormBuilder,private _http:ProcurmentService,private modalService: NgbModal,private toastr: ToastrService,private router:Router) { 
...
totalAmount=0
  calcAmount(item) { 
    ...
    item['netAmount1']= (item.quantity*item.costPrice).toFixed(2);
    if(item.discount)
