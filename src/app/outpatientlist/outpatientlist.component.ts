...
export class OutpatientlistComponent implements OnInit {
  ...
  constructor(private _http: OutpatientlistService, private modalService: NgbModal, private toastr: ToastrService) { }
 ...
 this.regId=data.regId
      ...
      this.modalService.open(basic3, { ariaLabelledBy: 'modal-basic-title' }).result.then((result) => {
        this.closeResult = `Closed with: ${result}`;
      }, (reason) => {
        this.closeResult = `Dismissed ${this.getDismissReason(reason)}`;
      },);
